...
ZED ROS2 wrapper is built from the [official zed-ros2-wrapper](https://github.com/stereolabs/zed-ros2-wrapper) repository, with the script under directory "docker" there by invoking the command:
```bash
./jetson_build_dockerfile_from_sdk_and_l4T_version.sh l4t-r36.4.0 zedsdk-5.1.0
```
Upon completion, your image repository:tag name is going to be used with the corresponding script. 
