...
FeatureTrackerTAPNext::trackImage(double cur_time, const cv::Mat &cur_img,
  ...
  ROS_INFO("TAPNext TrackImage undistort time: %f ms", t_r.toc());
  t_r.tic();
  ...
  ROS_INFO("TAPNextTRT Inference time: %f ms", t_r.toc());
  t_r.tic();
...
void FeatureTrackerTAPNext::resetTracker(const cv::Mat &cur_img) {
  ...
  while (xs_model.size() < static_cast<size_t>(params.tapnext_max_track)) {
