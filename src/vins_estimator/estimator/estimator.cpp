...
Estimator::Estimator(Parameters &params)
  ...
  if(params.video_mode) {
      std::string engine_path = params.video_depth_engine_path;
      depthInfererVideo = std::make_shared<DepthInferVideo>(engine_path);
      ROS_INFO("Initialized DepthInferVideo (Stateful)!");
  } else {
      std::string engine_path = params.depth_engine_path;
      depthInferer = std::make_shared<DepthInfer>(engine_path);
      ROS_INFO("Initialized DepthInfer (Stateless)!");
  }
...
void Estimator::inputImage(double t, const cv::Mat &_img, const cv::Mat &depth_img, 
  ...
  // Submit regardless of RGD flag, because Backend needs checking.
  if (params.video_mode && depthInfererVideo && solver_flag != INITIAL && params.use_depth && WEIGHT > 0) {
       // VINS-Mono already downsamples by 2 internally for optimization.

       if (inputImageCnt % 2 == 0) {
           std::cout << "[Estimator] Submitting frame " << std::fixed << t << " to async worker." << std::endl;
           depthInfererVideo->input_image(t, _img);
       } else {
           // std::cout << "[Estimator] Skipping frame " << t << " for inference (Decimation)" << std::endl;
       }
  }

  cv::Mat depth_8u;
  bool infer_ready = (params.video_mode ? (bool)depthInfererVideo : (bool)depthInferer);
  if (!infer_ready) ROS_WARN_THROTTLE(1, "Infer not ready: video=%d, videoPtr=%p, inferPtr=%p", params.video_mode, (void*)depthInfererVideo.get(), (void*)depthInferer.get());
  if (infer_ready && params.rgd && solver_flag != INITIAL){
...
void Estimator::smartDepthInitialization() {
    ...
    if (scale_is_initialized) {
        for (auto &it_per_id : f_manager.feature) {
            ...
            // Oracle initialization mode for depth fusion
            // When fusion is enabled, aggressively initialize any unsolved feature using dense depth
...
struct OptimizationDiagnostics {
  ...
  std::vector<ceres::ResidualBlockId> ordinal_block_ids;
  ...
  double ordinal_cost_initial = 0.0;
  double ordinal_cost_final = 0.0;
...
void Estimator::optimization() {
  ...
  TicToc t_opt;
  // Diagnostics tracker commented out for performance reasons
  // OptimizationDiagnostics diag;

  // Compute fused depth for all features before optimization
  ...
  if (last_marginalization_info && last_marginalization_info->valid) {
    ...
    // Marginalization block ID tracking commented out for performance
    // diag.marginalization_block_ids.push_back(block_id);
    // diag.num_marginalization_factors++;
    // diag.total_marginalization_residuals = last_marginalization_info->n;
  }
  if (params.use_imu) {
    for (int i = 0; i < frame_count; i++) {
      int j = i + 1;
      ...
      ceres::ResidualBlockId block_id = problem.AddResidualBlock(imu_factor, NULL, para_Pose[i],
                               ...
                               para_SpeedBias[j]);
      // IMU block ID tracking commented out for performance
      // diag.imu_block_ids.push_back(block_id);
      // diag.num_imu_factors++;
      // diag.total_imu_residuals += 15;
    }
  }
  ...
  for (auto &it_per_id : f_manager.feature) {
    ...
    // Feature optimization counter commented out for performance
    ...
    for (auto &it_per_frame : it_per_id.feature_per_frame) {
      ...
      if (imu_i != imu_j) {
        ...
        // Mono reprojection counters commented out for performance
        // diag.visual_block_ids.push_back(block_id);
        // diag.addBlockToCurrentFeature(block_id);
      ...
      if (params.stereo && it_per_frame.is_stereo) {
        ...
        if (imu_i != imu_j) {
          ...
          ceres::ResidualBlockId block_id = problem.AddResidualBlock(f, loss_function, para_Pose[imu_i],
                                   ...
                                   para_Td[0]);
          // Stereo reprojection counters commented out for performance
          // diag.visual_block_ids.push_back(block_id);
          // diag.addBlockToCurrentFeature(block_id);
          ...
          ceres::ResidualBlockId block_id = problem.AddResidualBlock(f, loss_function, para_Ex_Pose[0],
                                   para_Ex_Pose[1], para_Feature[feature_index],
                                   para_Td[0]);
          // One-frame reprojection counters commented out for performance
          // diag.visual_block_ids.push_back(block_id);
          // diag.addBlockToCurrentFeature(block_id);
    ...
    if (solver_flag == NON_LINEAR && params.use_depth && WEIGHT > 0.0 && scale_is_initialized) {
      ...
      // Early fusion block was moved later in the code to ensure proper data accumulation
            ...
            if (patch_vals.size() >= 5) {  // Need enough valid samples
              ...
              if (depth_consistency > 0.7f) {  // Patch is relatively uniform
                ...
                if (aligned_inv_depth > 0.01 && aligned_inv_depth < 10.0 &&  // Valid inv depth range
                  ...
                  // Always use the standard prior approach even when fusion is enabled.
                  // This allows oracle mode (fusion) to coexist with standard depth factors.
                  if (true) {
                  // else {
                      ...
                      // Oracle implementation mode:
                      // When fusion is enabled, we trust the dense depth map implicitly.
                      // Variance-gated fusion with complementary boosting
                      // 1. Get variance (using sample variance of multi-view observations)
                      ...
                      if (params.mv_depth_fusion) {
                          // Enforce hard rejection threshold without overriding should_add_prior flag
                      ...
                      if (should_add_prior) {
                        ...
                        // Using raw weight without normalization (as requested) 
...
// Accumulate statistics for debugging
                        ...
                        // Depth block ID tracking commented out for performance
                        // diag.depth_block_ids.push_back(block_id);
                        added_depth_factors++;
                        // diag.num_depth_prior_factors++;
                        // diag.total_depth_residuals++;
  ...
  // Photometric regularization approach
  // Note: Photometric regularization via numeric differentiation is 
  ...
  // =====================================================================
  // Trace ordinal constraint logic for debugging
  ...
  if (solver_flag == NON_LINEAR && params.ordinal_depth && params.use_depth && scale_is_initialized && marginalization_flag == MARGIN_OLD) {
    ...
    for (auto &it_per_id : f_manager.feature) {
      ...
      // Robustness filter: Only use temporally stable features for ordinal constraints
    ...
    // Track filter statistics for this iteration
    int iter_checks = 0;
    int iter_pass = 0;
    ...
    for (size_t i = 0; i < candidates.size() && ordinal_pairs.size() < static_cast<size_t>(params.ordinal_depth_max_pairs); i++) {
      for (size_t j = i + 1; j < candidates.size() && ordinal_pairs.size() < static_cast<size_t>(params.ordinal_depth_max_pairs); j++) {
        ...
        // Spatial distance filter: Only compare features within local neighborhood
        // to avoid global scale drift issues
        ...
        if (abs_diff > max_inv_depth * params.ordinal_depth_margin) {  // Significant relative difference
          ...
          if (diff > 0) {
            ...
            pair.inv_depth_closer = candidates[i].mono_inv_depth; // Store for adaptive margin calculation
          } else {
            // j is closer
            pair.feature_idx_closer = candidates[j].feature_index;
            pair.feature_idx_farther = candidates[i].feature_index;
            pair.inv_depth_closer = candidates[j].mono_inv_depth; // Store for adaptive margin calculation
          }
    ...
    for (int i = 0; i < pairs_to_add; i++) {
      ...
      // Ordinal block ID tracking commented out for performance
    ...
    // Print filter statistics for debugging
    ...
    if (added_ordinal_factors > 0) {
      ...
      // Find the frame with the most ordinal pairs to visualize
      ...
      // Count pairs
      // Only visualize the pairs that were actually added to the solver
  ...
  // Print aggregate gate statistics for debugging
  ...
  // Factor diagnostics before solving commented out for performance
  if (params.diagnostics) {
    // diag.print();
  }
  
  // Store pointer to para_Feature for per-feature analysis (commented out for performance)
  // diag.para_Feature_ptr = para_Feature;
  
  // Evaluate per-group costs before optimization (commented out for performance)
  if (params.diagnostics) {
  ...
  if (params.diagnostics) {
    ...
    // Per-group cost breakdown commented out for performance
    // diag.printCostBreakdown();
    
    // CSV export for visualization commented out for performance
